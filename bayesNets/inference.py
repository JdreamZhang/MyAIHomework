...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        #At first, we get all the Conditional Probability Tables
        CPTs =bayesNet.getAllCPTsWithEvidence(evidenceDict)
        #Then for each variable in the eliminationOrder, we need to eliminate the variable
        for variable in eliminationOrder:
            allFactors, joinedFactor= joinFactorsByVariable(CPTs,variable)
            ...
            CPTs=allFactors
        #Last, normalize the last newFactor
