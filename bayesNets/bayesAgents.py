...
def fillObsCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    #As the hint given, we first loop over possible house positions, then over possible walls for each house,
    #and finally over assignments to (wall color, ghost house position, food house position) triples. 
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar=OBS_VAR_TEMPLATE % obsPos
            #construct a factor (currentVar,parentVar,varDomainDict)
            ...
            for assignment in obsVarFactor.getAllPossibleAssignmentDicts():
                #print "------",assignment
                #set the probablity accoding to the house variable
                houseVal=getHouseValFromHousePosition(obsPos[0],obsPos[1],gameState)
                ...
                #set the probablity for this assignment
                if assignment[obsVar]== RED_OBS_VAL:
                    obsVarFactor.setProbability(assignment,red)
                if assignment[obsVar]==BLUE_OBS_VAL:
                    obsVarFactor.setProbability(assignment,blue)
                if assignment[obsVar]==NO_OBS_VAL:
                    obsVarFactor.setProbability(assignment,no)
            
            #add each obsVarFactor for each obsvAR
            bayesNet.setCPT(obsVar,obsVarFactor)



#define a fucntion which can get the house variable from the house position
def getHouseValFromHousePosition(a,b,gameState):
    width=gameState.data.layout.width
    height=gameState.data.layout.height

    if a>width/2.0 and b>height/2.0:
        return TOP_RIGHT_VAL
    elif a>width/2.0 and b<height/2.0:
